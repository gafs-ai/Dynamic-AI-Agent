"""Build toolcomponent as an extension module (.pyd on Windows, .so on Linux) with Nuitka.
...
The compiled module is placed under:
    build/<arch>/gafs/dynamicaiagent/toolcomponent/
...
PACKAGE = "gafs.dynamicaiagent.toolcomponent"
PACKAGE_DIR = "gafs/dynamicaiagent/toolcomponent"
BUILD_BASE = "build"
...
def main() -> int:
    # .../Python/gafs/dynamicaiagent/toolcomponent/build_nuitka.py -> Python/
    ...
    # Prefer matching toolcomponent.*.pyd/.so
    candidates = list(out_dir.glob("toolcomponent*.pyd")) + list(
        out_dir.glob("toolcomponent*.so")
    )
    if not candidates:
    ...
    # Copy the compiled module under build/<arch>/gafs/dynamicaiagent/toolcomponent
    target_dir = out_dir / "gafs" / "dynamicaiagent" / "toolcomponent"
    ...
    print("  pytest gafs/dynamicaiagent/toolcomponent/test/test_build_toolcomponent.py -v")
    return 0
