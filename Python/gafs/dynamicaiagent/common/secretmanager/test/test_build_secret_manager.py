"""Wrapper tests for running SecretManager tests against the Nuitka-compiled module.

This module:
- Adjusts `gafs.dynamicaiagent.common.__path__` so that
  `gafs.dynamicaiagent.common.secretmanager` is imported from the compiled artifact
  under `build/<arch>/gafs/dynamicaiagent/common/`.
- Then loads the existing source test module
  `gafs.dynamicaiagent.common.secretmanager.test.test_secret_manager`
...
def _adjust_common_path_for_compiled() -> None:
    ...
    if build_common_dir.is_dir():
        ...
        pytest.skip(f"Compiled secretmanager not found at {build_common_dir}")
