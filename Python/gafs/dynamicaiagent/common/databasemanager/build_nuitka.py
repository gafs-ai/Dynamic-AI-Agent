"""Build databasemanager as an extension module (.pyd on Windows, .so on Linux) with Nuitka.

Output is placed under build/<arch>/ (e.g. build/win_x64, build/win_arm64) so
that multiple architectures can be built side by side.
...
# Fully qualified package name and its relative path inside Python/.
PACKAGE = "gafs.dynamicaiagent.common.databasemanager"
PACKAGE_DIR = "gafs/dynamicaiagent/common/databasemanager"

# Base directory for all build outputs (relative to the Python/ root).
BUILD_BASE = "build"
...
def main() -> int:
    ...
    # Path: Python/gafs/dynamicaiagent/common/databasemanager/build_nuitka.py
    ...
    # Verify that the package source directory exists.
    ...
    candidates = (
        list(out_dir.glob("databasemanager*.pyd"))
        + list(out_dir.glob("databasemanager*.so"))
    )
    if not candidates:
