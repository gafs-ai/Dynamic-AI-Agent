...
def main() -> int:
    # .../Python/gafs/dynamicaiagent/common/databasemanager/build_nuitka.py -> Python/
    ...
    # Prefer matching databasemanager.*.pyd/.so
    candidates = list(out_dir.glob("databasemanager*.pyd")) + list(
        out_dir.glob("databasemanager*.so")
    )
    if not candidates:
    ...
    # We do not shadow the top-level `gafs` package; tests will adjust
    # `gafs.dynamicaiagent.common.__path__` as described in NuitkaBuildRules.md.
