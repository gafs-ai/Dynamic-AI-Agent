"""Pytest wrapper that runs the SurrealDB *local* provider test suite against
...
# parents[5] of this file (…/databaseprovider/test/test_build_databaseprovider.py)
...
_LOCAL_TEST_MODULE = (
    "gafs.dynamicaiagent.utils.databaseprovider.test.test_surrealdb_local_provider"
)
_local_source_path = (
    _PYTHON_ROOT
    / "gafs"
...
if _LOCAL_TEST_MODULE not in sys.modules:
    _spec = importlib.util.spec_from_file_location(_LOCAL_TEST_MODULE, _local_source_path)
    _mod = importlib.util.module_from_spec(_spec)
    sys.modules[_LOCAL_TEST_MODULE] = _mod
    _spec.loader.exec_module(_mod)

# ── Re-export all tests and fixtures so pytest can collect them ───────────────

from gafs.dynamicaiagent.utils.databaseprovider.test.test_surrealdb_local_provider import *  # noqa: F401, F403, E402
