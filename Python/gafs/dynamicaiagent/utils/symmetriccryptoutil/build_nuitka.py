"""Build symmetriccryptoutil as an extension module (.pyd on Windows, .so on Linux) with Nuitka.
...
PACKAGE = "gafs.dynamicaiagent.utils.symmetriccryptoutil"
PACKAGE_DIR = "gafs/dynamicaiagent/utils/symmetriccryptoutil"
BUILD_BASE = "build"
...
def main() -> int:
    # .../Python/gafs/dynamicaiagent/utils/symmetriccryptoutil/build_nuitka.py -> Python/
    ...
    # Prefer matching symmetriccryptoutil.*.pyd/.so (avoid picking other compiled modules).
    candidates = list(out_dir.glob("symmetriccryptoutil*.pyd")) + list(out_dir.glob("symmetriccryptoutil*.so"))
    if not candidates:
    ...
    # (we will NOT shadow the top-level `gafs` package; tests will adjust utils.__path__ instead)
    ...
    print("  pytest gafs/dynamicaiagent/utils/symmetriccryptoutil/test/test_build_symmetric_crypto_util.py -v")
    return 0
