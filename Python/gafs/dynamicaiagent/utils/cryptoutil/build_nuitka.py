"""Build cryptoutil as an extension module (.pyd on Windows, .so on Linux) with Nuitka.
...
# The fully qualified package name and its relative path inside Python/.
PACKAGE = "gafs.dynamicaiagent.utils.cryptoutil"
PACKAGE_DIR = "gafs/dynamicaiagent/utils/cryptoutil"
...
def main() -> int:
    ...
    # Path: Python/gafs/dynamicaiagent/utils/cryptoutil/build_nuitka.py
    ...
    candidates = (
        list(out_dir.glob("cryptoutil*.pyd"))
        + list(out_dir.glob("cryptoutil*.so"))
    )
    if not candidates:
    ...
    # Copy the compiled module to build/<arch>/gafs/dynamicaiagent/utils/
    # so that tests can load it by adjusting gafs.dynamicaiagent.utils.__path__.
