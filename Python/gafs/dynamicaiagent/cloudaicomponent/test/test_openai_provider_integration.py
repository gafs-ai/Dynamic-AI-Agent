...
Runs against secret_test_config_openai_*.json config files.
...
async def test_openai_provider_chat_text():
    ...
    response = await OpenAiProvider.invoke(options, request)
    assert response.output is not None
    assert response.output.messages
    assert isinstance(response.output.messages[0].content, str)
    _log_response_content("openai_chat_text", response)
...
async def test_openai_provider_chat_image():
    ...
    assert response.output.messages
    assert isinstance(response.output.messages[0].content, str)
    _log_response_content("openai_chat_image", response)
...
async def test_openai_provider_embedding():
    ...
    _log_response_content("openai_embedding", response)


@pytest.mark.asyncio
async def test_openai_provider_speech_to_text():
    pytest.skip("Speech-to-text requires provider-specific payload conversion (not yet implemented)")


@pytest.mark.asyncio
async def test_openai_provider_text_to_speech_not_implemented():
    options = AiConnectionParameters(
        operation_type=AiOperationType.TEXT_TO_SPEECH,
        deployment_type=AiDeploymentType.CLOUD,
        provider_type=CloudAiProviderType.OPENAI,
    )
    options.parameters = {"model": "tts-1", "api_key": "key"}
    ...
    with pytest.raises(CloudAiUnsupportedOperationException):
        await OpenAiProvider.invoke(options, request)


@pytest.mark.asyncio
async def test_openai_provider_image_generation_not_implemented():
    options = AiConnectionParameters(
        operation_type=AiOperationType.IMAGE_GENERATION,
        deployment_type=AiDeploymentType.CLOUD,
        provider_type=CloudAiProviderType.OPENAI,
    )
    options.parameters = {"model": "dall-e-3", "api_key": "key"}
    from gafs.dynamicaiagent.modelcomponent.models.ai_payload import TextCompletionPayload
    payload = TextCompletionPayload()
    payload.text = "a cat"
