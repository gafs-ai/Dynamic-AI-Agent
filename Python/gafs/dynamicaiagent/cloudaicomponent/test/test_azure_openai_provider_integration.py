...
Tests real API calls against Azure OpenAI using config files in this directory.
Each test loads its credentials from a secret_test_config_azure_openai_*.json file
...
def _make_azure_connection(cfg: dict, operation_type: AiOperationType) -> AiConnectionParameters:
    """Build AiConnectionParameters for Azure OpenAI from a config dict."""
...
async def test_azure_openai_provider_chat_text():
    ...
    response = await AzureOpenAiProvider.invoke(conn, request)
    ...
    _log_response("azure_chat_text", response)
...
async def test_azure_openai_provider_chat_image():
    ...
    assert isinstance(response.output.messages[0].content, str)
    assert len(response.output.messages[0].content) > 0
    _log_response("azure_chat_image", response)
...
async def test_azure_openai_provider_embedding():
    ...
    _log_response("azure_embedding", response)


@pytest.mark.asyncio
async def test_azure_openai_provider_chat_text_streaming():
    ...
    assert response.output.messages
    assert isinstance(response.output.messages[0].content, str)
    _log_response("azure_chat_text_stream", response)
...
@pytest.mark.asyncio
async def test_azure_openai_provider_text_to_speech_raises():
    ...
    with pytest.raises(CloudAiUnsupportedOperationException):
        await AzureOpenAiProvider.invoke(conn, request)


@pytest.mark.asyncio
async def test_azure_openai_provider_image_generation_raises():
...
@pytest.mark.asyncio
async def test_azure_openai_provider_speech_to_text_raises():
...
@pytest.mark.asyncio
async def test_azure_openai_provider_missing_endpoint_raises():
    """Missing endpoint raises CloudAiConfigurationException."""
...
@pytest.mark.asyncio
async def test_azure_openai_provider_missing_api_key_raises():
...
@pytest.mark.asyncio
async def test_azure_openai_provider_null_payload_raises():
