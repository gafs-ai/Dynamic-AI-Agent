"""OpenAiProvider — ICloudAiProvider implementation for the standard OpenAI API."""
...
class OpenAiProvider(ICloudAiProvider):
    ...
    @classmethod
    def _get_client(cls, connection_parameters: AiConnectionParameters) -> AsyncOpenAI:
        """Build and return an AsyncOpenAI client from connection_parameters.

        Raises:
            CloudAiConfigurationException: If api_key is missing.
        """
        ...
        org = connection_parameters.parameters.get("organization_id")
        if org:
    ...
    async def invoke(
        ...
        if op_val == AiOperationType.SPEECH_TO_TEXT.value:
            raise CloudAiUnsupportedOperationException(
                message="Speech-to-text is not implemented by OpenAiProvider.",
                provider=CloudAiProviderType.OPENAI,
            )
        if op_val == AiOperationType.TEXT_TO_SPEECH.value:
            raise CloudAiUnsupportedOperationException(
                message="Text-to-speech is not implemented by OpenAiProvider.",
                provider=CloudAiProviderType.OPENAI,
            )
        if op_val == AiOperationType.IMAGE_GENERATION.value:
            raise CloudAiUnsupportedOperationException(
                message="Image generation is not implemented by OpenAiProvider.",
                provider=CloudAiProviderType.OPENAI,
            )
        raise CloudAiUnsupportedOperationException(
