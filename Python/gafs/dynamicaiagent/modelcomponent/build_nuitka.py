...
Run from the project Python/ directory:
    ...
    python gafs/dynamicaiagent/modelcomponent/build_nuitka.py

Requires: `pip install nuitka` and a C compiler.

The compiled module is placed under:
    build/<arch>/gafs/dynamicaiagent/modelcomponent/
...
def main() -> int:
    ...
    # Prefer matching modelcomponent.*.pyd/.so
    candidates = list(out_dir.glob("modelcomponent*.pyd")) + list(
        out_dir.glob("modelcomponent*.so")
    )
    if not candidates:
    ...
    # Copy the compiled module under build/<arch>/gafs/dynamicaiagent/modelcomponent
    target_dir = out_dir / "gafs" / "dynamicaiagent" / "modelcomponent"
    ...
    print("  pytest gafs/dynamicaiagent/modelcomponent/test/test_build_modelcomponent.py -v")
    return 0
